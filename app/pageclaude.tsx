...
import {
  ...
  BarChart2, Inbox, Users, Link2, QrCode, Camera
} from 'lucide-react';

// ─── NEW: SCANNING OVERLAY COMPONENT ──────────────────────────────────────────
function ScanningTrigger({ active, onComplete }: { active: boolean; onComplete: () => void }) {
  useEffect(() => {
    if (active) {
  ...
  return (
    <div className="absolute inset-0 z-50 bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center p-6 animate-in fade-in duration-500">
      <div className="relative mb-8">
        {/* The "Physical" QR Code on a sign/truck */}
        <div className="w-32 h-32 bg-white border-4 border-slate-900 rounded-2xl flex items-center justify-center shadow-xl">
          <QrCode size={80} className="text-slate-900" />
          <div className="absolute -top-3 -right-3 bg-blue-600 text-white text-[10px] font-black px-2 py-1 rounded-lg shadow-lg animate-bounce">
            SCAN ME
          </div>
        </div>

        {/* The "Hand" holding a phone scanning */}
        <div className="absolute -bottom-6 -right-16 animate-in slide-in-from-bottom-12 slide-in-from-right-12 duration-700">
          <div className="w-24 h-44 bg-slate-800 rounded-[1.5rem] border-[3px] border-slate-700 shadow-2xl rotate-[-12deg] overflow-hidden relative">
            <div className="absolute inset-0 bg-blue-500/10 flex items-center justify-center">
              <div className="w-12 h-12 border-2 border-white/50 rounded-lg animate-ping" />
              <Camera size={20} className="text-white/80" />
            </div>
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-8 h-1 bg-slate-600 rounded-full" />
          </div>
        </div>
      </div>
      
      <div className="text-center">
        <p className="text-xs font-black text-slate-900 uppercase tracking-[0.2em] mb-1">Detecting Link</p>
        <p className="text-[10px] text-slate-400 font-medium">Opening your custom intake form...</p>
      </div>
    </div>
  );
}

// ─── ANIMATED FORM → BOARD DEMO (v2 with Scanning & Steps) ──────────────────
function FormToBoardDemo() {
  const [phase, setPhase] = useState<'scanning' | 'filling' | 'flying' | 'board' | 'pause'>('scanning');
  ...
  useEffect(() => {
    ...
    if (phase === 'filling') {
      if (fieldIndex < fields.length) {
        ...
        if (f.isSelect) {
          ...
          timeoutRef.current = setTimeout(() => { setFieldIndex(i => i + 1); setCharIndex(0); }, 250);
        }
      } else {
    ...
    if (phase === 'flying') {
      timeoutRef.current = setTimeout(() => { 
        setPhase('board'); 
        setTimeout(() => setNewCardVisible(true), 150); 
      }, 800);
    }
    if (phase === 'board') {
      timeoutRef.current = setTimeout(() => setPhase('pause'), 4000);
    }
    if (phase === 'pause') {
      timeoutRef.current = setTimeout(() => {
        setPhase('scanning'); setFieldIndex(0); setCharIndex(0);
  ...
  return (
    <div className="relative">
      <style>{`
        @keyframes flyAcross { 0%{transform:translate(0,0) rotate(0deg);opacity:1} 50%{transform:translate(30vw,-40px) rotate(15deg);opacity:1} 100%{transform:translate(60vw,0) rotate(0deg);opacity:0} }
        @keyframes cardDrop  { 0%{opacity:0;transform:translateY(-20px) scale(0.9)} 100%{opacity:1;transform:translateY(0) scale(1)} }
        @keyframes pulseGlow { 0%,100%{box-shadow: 0 0 0 0 rgba(37, 99, 235, 0.4)} 50%{box-shadow: 0 0 0 8px rgba(37, 99, 235, 0)} }
        .plane-anim { animation:flyAcross 0.8s cubic-bezier(.25,.46,.45,.94) forwards; position:absolute; top:40%; left:10%; z-index:40; }
        .card-drop  { animation:cardDrop 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; }
        .step-glow { animation: pulseGlow 2s infinite; }
      `}</style>

      {/* PROCESS TRACKER HEADERS */}
      <div className="grid grid-cols-3 gap-4 mb-10 relative">
        <div className="absolute top-4 left-0 w-full h-[2px] bg-slate-100 -z-10 hidden md:block" />
        {[
          { id: 1, label: 'Scan to Open', active: phase === 'scanning' || phase === 'filling', done: phase === 'flying' || isBoard, icon: <QrCode size={14}/> },
          { id: 2, label: 'Instant Send', active: phase === 'flying', done: isBoard, icon: <Send size={14}/> },
          { id: 3, label: 'Lead Board', active: isBoard, done: false, icon: <CheckCircle size={14}/> }
        ].map((s) => (
          <div key={s.id} className="flex flex-col items-center">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center border-2 transition-all duration-500 mb-2 bg-white ${s.active ? 'border-blue-600 text-blue-600 scale-110 shadow-lg step-glow' : s.done ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-200 text-slate-300'}`}>
              {s.done ? <Check size={18} strokeWidth={3} /> : s.icon}
            </div>
            <span className={`text-[10px] font-black uppercase tracking-tighter transition-colors ${s.active ? 'text-slate-900' : 'text-slate-400'}`}>{s.label}</span>
          </div>
        ))}
      </div>

      <div className="grid md:grid-cols-2 gap-6 relative overflow-hidden bg-slate-50/50 p-6 rounded-[2.5rem] border border-slate-200/60">
        {phase === 'flying' && (
        ...
        <div className={`relative transition-all duration-500 ${isBoard ? 'opacity-30 grayscale scale-95' : 'opacity-100'}`}>
          <div className="bg-slate-900 rounded-[2.5rem] p-2.5 border-[6px] border-slate-800 shadow-2xl mx-auto w-full max-w-[260px] overflow-hidden relative">
            
            {/* SCANNING OVERLAY TRIGGER */}
            <ScanningTrigger 
              active={phase === 'scanning'} 
              onComplete={() => setPhase('filling')} 
            />

            <div className="bg-white rounded-[1.8rem] overflow-hidden min-h-[400px]">
               <div className="bg-slate-50 border-b border-slate-100 p-4">
                  <div className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Quote Request</div>
               </div>
               <div className="p-4 space-y-3">
                  {fields.map((f, i) => (
                    <div key={i}>
                      <div className="text-[8px] font-bold text-slate-400 uppercase mb-1">{f.label}</div>
                      <div className={`h-8 border rounded-lg flex items-center px-2 text-[10px] transition-all ${fieldIndex === i && phase === 'filling' ? 'border-blue-500 ring-2 ring-blue-50 bg-blue-50/10' : 'border-slate-100 text-slate-800'}`}>
                        {displayedValues[i]}
                        {fieldIndex === i && phase === 'filling' && <div className="w-0.5 h-3 bg-blue-500 ml-1 animate-pulse"/>}
                      </div>
                    </div>
                  ))}
                  <div className={`h-10 rounded-xl flex items-center justify-center text-xs font-bold transition-all ${fieldIndex >= fields.length ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-100 text-slate-400'}`}>
                    {phase === 'flying' ? 'Sending...' : 'Submit Request →'}
                  </div>
               </div>
            </div>
          </div>
        ...
        {/* CONTRACTOR BOARD VIEW */}
        <div className={`transition-all duration-500 ${isBoard ? 'opacity-100 scale-100' : 'opacity-30 grayscale scale-95'}`}>
          <div className="bg-white border border-slate-200 rounded-[2rem] shadow-xl h-full overflow-hidden min-h-[412px]">
             <div className="bg-slate-900 p-4 flex items-center justify-between">
                <span className="text-[10px] font-bold text-white uppercase tracking-widest">Lead Board</span>
             ...
             <div className="p-4 space-y-3 bg-slate-50 h-full">
                {newCardVisible && (
                  <div className="card-drop bg-white border-2 border-blue-500 rounded-2xl p-4 shadow-xl relative overflow-hidden">
                    <div className="absolute top-0 right-0 bg-blue-500 text-white text-[8px] font-black px-2 py-1 rounded-bl-lg uppercase">New Request</div>
                    <div className="text-[10px] font-bold text-slate-400 uppercase mb-1">Roofing</div>
                    <div className="text-sm font-black text-slate-900 leading-none mb-1">Mike Torres</div>
                    <div className="text-[10px] text-slate-500 line-clamp-1 italic">"Storm damage - missing shingles"</div>
                    <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between">
                       <span className="text-[10px] font-bold text-blue-600">Just Now</span>
                       <span className="text-[10px] font-bold text-slate-900">$2,400 est.</span>
                    </div>
                  </div>
                )}
                {existingLeads.map((lead, i) => (
                   <div key={i} className="bg-white border border-slate-200 rounded-2xl p-4 opacity-60">
                      <div className="flex justify-between items-start mb-2">
                        <div className="text-[8px] font-bold text-slate-400 uppercase">{lead.cat}</div>
                        <div className={`text-[8px] font-bold px-1.5 py-0.5 rounded ${lead.sc}`}>{lead.status}</div>
                      </div>
