      ...
      <div className={`bg-white rounded-xl overflow-hidden border transition-all duration-200 ${
        ...
        <div className={`px-5 py-4 flex items-center justify-between transition-colors duration-200 ${
          ...
          <div className="flex items-center gap-3">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
              ...
              <Mail className={`w-4 h-4 ${digest.enabled ? 'text-indigo-600' : 'text-gray-400'}`} />
            </div>
            <div>
              <p className={`text-sm font-bold ${digest.enabled ? 'text-indigo-900' : 'text-gray-800'}`}>
                Daily Digest Email
              </p>
              <p className="text-xs text-gray-400 mt-0.5">
                {digest.enabled
                  ? `On · sends to ${recipientLabels[digest.recipient] || company.email}`
        ...
        <div className="p-5 space-y-5">
          ...
          <p className="text-sm text-gray-600 leading-relaxed">
            A morning summary delivered at <strong>7:00 AM</strong> of everything that needs your attention. Only sends on days when there's actually something to act on — no noise on quiet days.
          </p>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {[
                ...
                'Overdue payments',
                'Jobs done, payment not recorded',
