    ...
    <>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="p-5 grid grid-cols-2 sm:grid-cols-3 gap-4">
          ...
          <div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1">Email</p><p className="text-sm font-semibold text-indigo-600 truncate">{lead.email}</p></div>
          <div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1">Phone</p><p className="text-sm font-semibold text-indigo-600">{lead.phone}</p></div>
          {lead.address_line_1 && (
          ...
          <div>
            ...
            <span className="inline-flex items-center px-2.5 py-1 bg-indigo-50 border border-indigo-100 text-xs font-bold text-indigo-600 rounded-lg">{lead.category}</span>
          </div>
        </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-100">
              ...
              <div className="flex gap-2">
                {lead.file_urls.slice(0, 4).map((_: any, i: number) => (
                  <div key={i} className="w-12 h-12 bg-gradient-to-br from-indigo-100 to-purple-100 rounded-xl border border-indigo-200 flex items-center justify-center">
