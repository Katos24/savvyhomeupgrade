...
export default function CreateLeadModal({
  ...
  // Search vs Manual Entry mode state
  ...
  useEffect(() => {
    if (!isSearchingCustomer || customerQuery.trim().length < 2) {
      setCustomerResults([]);
      return;
    }
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/company/${companySlug}/customers/search?q=${encodeURIComponent(customerQuery)}`,
  ...
  }, [customerQuery, companySlug, isSearchingCustomer]);
          ...
          <div className="flex items-start justify-between mb-6">
            <div>
              <h2 className={`text-2xl font-bold tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>
                {isSearchingCustomer ? 'Find Existing Customer' : 'Quick Add Lead'}
              </h2>
              <p className="text-xs uppercase tracking-widest font-bold mt-1.5" style={{ color: accentColor }}>
                {isSearchingCustomer ? 'Search Database' : 'Direct Entry'}
              </p>
            </div>
            <button 
              onClick={onClose} 
            ...
            <div className="space-y-4 animate-in fade-in duration-200">
              <div className="relative">
                ...
                <input
                  ...
                  className={`${inputClass} pl-12 pr-10 focus:ring-2 focus:ring-offset-0 focus:border-transparent`}
                  style={{ '--tw-ring-color': `${accentColor}80` } as React.CSSProperties}
                />
                {searchingCustomers && (
                  <Loader2 className={`absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 animate-spin ${isDark ? 'text-slate-400' : 'text-slate-500'}`} />
                )}
              </div>

              {/* Customer Results List */}
              <div className="min-h-[200px] max-h-[320px] overflow-y-auto custom-scrollbar">
                {customerQuery.trim().length >= 2 ? (
                  customerResults.length > 0 ? (
                    <div className={`divide-y rounded-2xl border overflow-hidden ${isDark ? 'divide-white/10 border-white/10 bg-white/5' : 'divide-slate-100 border-slate-200 bg-slate-50/50'}`}>
                      {customerResults.map((c) => (
                        <button
                          ...
                          className={`w-full text-left p-4 transition-colors flex items-center justify-between ${
                            isDark ? 'hover:bg-white/10' : 'hover:bg-slate-100/80'
                          }`}
                        >
                          <div>
                            <p className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{c.name}</p>
                            <p className={`text-xs mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                              {[c.email, c.phone].filter(Boolean).join(' · ')}
                            </p>
                          </div>
                          <span className="text-xs font-semibold px-2.5 py-1 rounded-lg" style={{ backgroundColor: `${accentColor}20`, color: accentColor }}>
                            Select
                          </span>
                        </button>
                      ))}
                    ...
                    <div className="text-center py-10">
                      <p className={`text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>No customers found</p>
                      <p className={`text-xs mt-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Try searching a different name, email, or number</p>
                    </div>
                  ) : null
                ) : (
                  <div className="text-center py-10">
