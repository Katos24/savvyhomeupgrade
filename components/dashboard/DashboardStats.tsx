  ...
  return (
    <section className="grid grid-cols-4 gap-1.5 sm:gap-2 mb-4 sm:mb-5 w-full">
      {stats.map((s, i) => (
        <div
          key={i}
          className={`rounded-lg px-2 py-1.5 sm:px-3 sm:py-2 transition-all min-w-0 ${
            isDark
              ? `bg-white/[0.025] border ${s.accent ? 'border-amber-500/20' : 'border-white/[0.06]'}`
              : `bg-white border ${s.accent ? 'border-amber-200' : 'border-slate-100'} shadow-[0_1px_4px_rgba(0,0,0,0.02)]`
          }`}
        >
