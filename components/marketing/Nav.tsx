...
export default function Nav() {
  ...
  return (
    <>
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <Link
              ...
              className={`flex items-center gap-2 text-sm font-black px-5 py-2.5 rounded-xl transition-all active:scale-95 ${
                scrolled
                  ? 'bg-[#1a6645] text-white hover:bg-[#144d34]'
      ...
      {/* MOBILE DRAWER */}
      <div
        className={`fixed top-0 right-0 z-[70] h-full w-[80%] max-w-[320px] bg-[#0d0d1a] border-l border-white/5 transition-transform md:hidden flex flex-col ${
      ...
      >
        ...
        <div className="px-4 pb-8 pt-4 flex flex-col gap-3 border-t border-white/5">
          <Link
            href="/signup"
            onClick={() => setMobileOpen(false)}
            className="flex items-center justify-center gap-2 py-4 rounded-xl bg-[#1a6645] text-white font-black"
          >
            Sign Up Today
            <ArrowRight size={18} />
          </Link>
