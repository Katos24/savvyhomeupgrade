...
export default function FeaturesSlider() {
  ...
  const overrideTimer = useRef<NodeJS.Timeout | null>(null);

  // ── SCROLL-TRIGGERED AUTO-ADVANCE ──
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { threshold: [0.1, 0.2, 0.3, 0.4, 0.55, 0.7, 0.85] }
    );
  ...
  return (
    <section 
      ref={sectionRef}
      id="features" 
      className={`relative transition-colors duration-700 overflow-hidden py-14 sm:py-32 scroll-mt-16 ${current.dark ? 'bg-[#0a0f1e]' : 'bg-white'}`}
    >
      <div className="relative max-w-7xl mx-auto px-4 sm:px-8">
        
        {/* TAB CONTROLS */}
        <div className="flex gap-2 sm:gap-4 mb-8 sm:mb-12 justify-center lg:justify-start">
          {slides.map((slide, idx) => (
            <button
              key={slide.id}
              onClick={() => handleTabClick(idx)}
              className={`px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest transition-all ${
                activeTab === idx 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-24 items-center">
          ...
          <div className={`flex flex-col order-2 ${activeTab === 0 ? 'lg:order-2' : 'lg:order-1'}`}>
            <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border mb-4 sm:mb-6 self-start ${current.dark ? 'border-emerald-500/30 bg-emerald-500/10' : 'bg-emerald-50 border-emerald-100'}`}>
              {current.icon}
            ...
            <h2 className={`font-black tracking-tight leading-[0.95] mb-4 sm:mb-6 ${current.dark ? 'text-white' : 'text-slate-900'}`} style={{ fontSize: 'clamp(1.75rem, 5vw, 4rem)' }}>
              {current.title}
            </h2>

            <p className={`text-[15px] sm:text-lg font-medium leading-relaxed mb-8 sm:mb-10 max-w-xl ${current.dark ? 'text-white/60' : 'text-slate-600'}`}>
              {current.desc}
            </p>
            ...
            <div className="grid grid-cols-1 gap-4 sm:gap-6">
              {current.features.map((f, i) => (
                <div key={i} className="flex items-center gap-3 sm:gap-4 group">
                  ...
                  <h4 className={`font-bold text-xs sm:text-sm uppercase tracking-wide ${current.dark ? 'text-white' : 'text-slate-900'}`}>{f.title}</h4>
                </div>
              ))}
            </div>
            ...
            <div className="flex gap-3 sm:gap-4 mt-8 sm:mt-12">
              <button 
                ...
                className={`p-3 sm:p-4 rounded-full transition-all border ${current.dark ? 'border-white/10 text-white hover:bg-white/5' : 'border-slate-200 text-slate-900 hover:bg-slate-50'}`}
              >
                <ChevronLeft size={18} />
              </button>
              <button 
                onClick={() => handleTabClick(activeTab === 0 ? 1 : 0)}
                className={`flex items-center gap-2 sm:gap-3 px-5 sm:px-8 py-3 sm:py-4 rounded-full font-black text-[10px] sm:text-xs uppercase tracking-widest transition-all ${current.dark ? 'bg-emerald-500 text-white hover:bg-emerald-400' : 'bg-slate-900 text-white hover:bg-slate-800'}`}
              >
          ...
          {/* VISUAL BLOCK — ORIGINAL, UNTOUCHED */}
